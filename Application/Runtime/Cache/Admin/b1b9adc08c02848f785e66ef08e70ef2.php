        ...
        <div class="container-fluid">
          <div class="info-center">
            <div class="page-header">
                <div class="pull-left">
						        <h4><a href="<?php echo U('officweb/newslist');?>">模块列表</a>&nbsp;>&nbsp;新增导航</h4>      
					      </div>
            </div>
            <div class="clearfix"></div>
            <div class="table-margin">
              <div class="col-lg-6">
                <form role="form" method="post" action="<?php echo U('officweb/newsadd');?>">
                    <div class="form-group">
                        <label>所属类别</label>
                        <select class="form-control" name="datatype">
                            <?php if(is_array($navlist)): foreach($navlist as $key=>$v): ?><option value="<?php echo ($v["navid"]); ?>" <?=$v['navid']==$info['id']?'selected':''?>><?php echo ($v["navname"]); ?></option><?php endforeach; endif; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>文章标题</label>
                        <input class="form-control" name="navname" value="<?php echo ($info["title"]); ?>">
                        <p class="help-block"></p>
                    </div>
                    <div class="form-group">
                        <label>文章内容</label>
                            <script id="container" name="content" type="text/plain" style="height:250px;"><?php echo ($info["content"]); ?></script>
                        <!-- 配置文件 -->
